...
local function set_rules(msg, data)
	...
	return 'تم اظافة القوانين to:\n'..rules
end

local function get_rules(msg, data)
    local data_cat = 'rules'
    if not data[tostring(msg.to.id)][data_cat] then
		return 'لم يتم اظافة قانون بعد'
	end
...
local function lock_group_photo(msg, data)
	...
	return 'ارجو منك ان ترسل صورة المجموعة الان'
end
...
local function set_group_photo(msg, success, result)
  ...
  if success then
    ...
    save_data(_config.moderation.data, data)
    send_large_msg(receiver, 'تم حفظ صورة', ok_cb, false)
  else
...
function run(msg, matches)
    ...
    if data[tostring(msg.to.id)] then
		...
		if matches[1] == 'setphoto' and is_momod(msg) then
		    data[tostring(msg.to.id)]['settings']['set_photo'] = 'waiting'
	        save_data(_config.moderation.data, data)
	        return 'ارسل صوره المراد تغييرها هنا الان'
		end
